...
def format_advanced(df,pos):

    df.columns = df.columns.droplevel()

    df.drop(['G','Age','Tm','Pos','No.','GS','Rec','Yds','1D','Rush','YBC','YAC','Drop','BrkTkl'],axis=1,inplace=True)
        
    # Convert Int to Int%
    df['Int'] = df['Int']/df['Tgt']

    # Convert BrkTkl
    df['Att/Br'] = 1/df['Att/Br']
    df['Rec/Br'] = 1/df['Rec/Br']

    df = df.rename(columns={"Int": "Int%",
                            "YBC/R": "YBC/Rec",
                            "YAC/R": "YAC/Rec",
                            "Att/Br": "BrkTkl/Att",
                            "Rec/Br": "BrkTkl/Rec",
                            "Rat": "Passer Rating when Targeted"})
...
def normalize_career(df,years):
    ...
    df['YBC/Att_career'] = df['YBC/Att_career']/years
    df['YAC/Att_career'] = df['YAC/Att_career']/years
    df['BrkTkl/Att_career'] = df['BrkTkl/Att_career']/years
    df['YBC/Rec_career'] = df['YBC/Rec_career']/years
    df['YAC/Rec_career'] = df['YAC/Rec_career']/years
    df['ADOT_career'] = df['ADOT_career']/years
    df['BrkTkl/Rec_career'] = df['BrkTkl/Rec_career']/years
    df['Drop%_career'] = df['Drop%_career']/years
    df['Int%_career'] = df['Int%_career']/years
    df['Passer Rating when Targeted_career'] = df['Passer Rating when Targeted_career']/years
    ...
    if(0 not in df['Touch_career'].values):
...
def visit_player_page(name,url):
    try:
        ...
        for each in comments:
            if 'table' in str(each):
                try:
                    tables.append(pd.read_html(str(each), attrs = {'id': 'detailed_receiving_and_rushing'})[0])
                    break
                except:
                    continue
        ...
        for each in comments:
            if 'table' in str(each):
                try:
                    tables.append(pd.read_html(str(each), attrs = {'id': 'detailed_rushing_and_receiving'})[0])
                    break
                except:
                    continue
    ...
    table2 = format_advanced(table2,pos)

    # Merge normal and advanced tables
    table = table.merge(table2,how='right',on='Year')
    ...
    if(num_rows > 1):
        for i in range(num_rows-1):
            ...
            fp = (table['Rec'].iat[i+1] * ppr) + (table['RRTD'].iat[i+1] * 6) + (table['YScm'].iat[i+1] /10) + ((table['Rush 1D'].iat[i+1] + table['Rec 1D'].iat[i+1])*ppfd)
...
#train_buffer, test_buffer = visit_player_page('Jonathan Taylor','https://www.pro-football-reference.com/players/T/TaylJo02.htm')
#train_buffer, test_buffer = visit_player_page('Damiere Byrd','https://www.pro-football-reference.com/players/B/ByrdDa00.htm')
