...
def visit_player_page(name,url):
    ...
    return(train_df,test_df,position)
...
def post_processing(train_buffer,test_buffer):
    # Post-processing
...
for i in range(num_rows):
    ...
    if((i == 0) or (rb == False) or (wr == False)):
        train_df, test_df,position = visit_player_page(table['Player'].iat[i],table['hyper_link'].iat[i])

        if(position == 'rb'):
            rb_train_buffer = train_df
            rb_test_buffer = test_df

            rb = True
        else:
            wr_train_buffer = train_df
            wr_test_buffer = test_df
        ...
        if(position == 'rb'):
            rb_train_buffer = pd.concat([rb_train_buffer, train_df],ignore_index=True,axis=0)
            rb_test_buffer = pd.concat([rb_test_buffer, test_df],ignore_index=True,axis=0)
        else:
            wr_train_buffer = pd.concat([wr_train_buffer, train_df],ignore_index=True,axis=0)
            wr_test_buffer = pd.concat([wr_test_buffer, test_df],ignore_index=True,axis=0)
        
rb_train_buffer,rb_test_buffer = post_processing(rb_train_buffer,rb_test_buffer)
wr_train_buffer,wr_test_buffer = post_processing(wr_train_buffer,wr_test_buffer)

rb_train_buffer.to_csv('rb_train.csv',index=False)
rb_test_buffer.to_csv('rb_test.csv',index=False)

wr_train_buffer.to_csv('wr_train.csv',index=False)
wr_test_buffer.to_csv('wr_test.csv',index=False)

# Generatre projections
exec(open("predict_by_pos.py").read())
